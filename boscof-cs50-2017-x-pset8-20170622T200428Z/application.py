...
def articles():
    ...
    #check if geo has been set
    if not geo:
        raise RuntimeError("Geo not set")
    
    #look up articles using the lookup function
    articles = lookup(geo)
    
    # list up to 5 articles as a JSON object
    if len(articles)>5:
...
def search():
    ...
    #search database for any postal code, state or city name that starts with q
    places = db.execute("SELECT * FROM places WHERE postal_code LIKE :q OR place_name LIKE :q OR admin_name1 LIKE :q",q=q)
    
    #list up to 10 places as a JSON object
    if len(places)>10:
